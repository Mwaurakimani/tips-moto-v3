...
interface MatchDetailViewProps {
  ...
  onBack: () => void;
  onSave: (matchId: number, updatedMatch: Partial<{
    league: string;
    homeTeam: string;
    awayTeam: string;
    date: string;
...
export function MatchDetailView({ match, onBack, onSave, onTipsUpdate }: MatchDetailViewProps) {
  ...
  const [editedTip, setEditedTip] = useState<any>(null);
  ...
  const handleTipInputChange = (field: string, value: string | boolean) => {
    setEditedTip(prev => ({
      ...prev,
      [field]: value
    }));
