...
import { ArrowLeft, Edit, Save, X, Calendar, DollarSign, Target, Clock, Users, Info, Trash2, Plus, ChevronUp, ChevronDown, Copy } from 'lucide-react';
...
export function PackageDetailView({
  ...
  const [selectedTipsToAdd, setSelectedTipsToAdd] = useState<number[]>([]);
  ...
  const handleTipChange = (tipIndex: number, field: string, value: any) => {
  ...
  const handleTipSelectionToggle = (tipId: number) => {
  ...
  const handleRemoveTip = (tipIndex: number) => {
    setEditedTips(prev => prev.filter((_, index) => index !== tipIndex));
  };

  const handleMoveTip = (tipIndex: number, direction: 'up' | 'down') => {
    setEditedTips(prev => {
      const newTips = [...prev];
  ...
  const handleDuplicateTip = (tipIndex: number) => {
            ...
            <CardContent className="space-y-4">
              ...
              {isEditing ? (
                <div className="space-y-2">
                  <Label htmlFor="name" className="text-sm text-gray-600 dark:text-gray-400">Package Name</Label>
                  <Input
                    id="name"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <p className="font-medium text-black dark:text-white">{subscription.date}</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {isEditing ? (
                    ...
                    getDescriptionBadge(displaySubscription.description)
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {isEditing ? (
                    ...
                    <Badge variant="outline">{displaySubscription.duration}</Badge>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {isEditing ? (
                    <Input
                      type="number"
                      value={editedSubscription.tips}
                      onChange={(e) => handleInputChange('tips', parseInt(e.target.value) || 0)}
                      className="w-full"
                      min="1"
                    />
                  ) : (
                    <p className="font-medium text-black dark:text-white">{displaySubscription.tips}</p>
                  )}
                </div>
            ...
            <CardContent>
              <div className="space-y-4">
                <p className="text-gray-600 dark:text-gray-400 leading-relaxed">
                  This {displaySubscription.description} package provides {displaySubscription.tips} carefully selected tips
                  {displaySubscription.duration === 'Daily' ? ' every day' : ' every week'}. Our expert analysts
                  use advanced statistical models and insider knowledge to deliver high-quality predictions
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-4 max-h-96 overflow-y-auto">
                    {(isEditingTips ? editedTips : (subscription.tipsData?.slice(0, 10) || [])).map((tip, index) => (
                      <div key={tip.id || index} className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border">
                        {isEditingTips ? (
                          <div className="space-y-4">
                            <div className="flex items-center justify-between">
