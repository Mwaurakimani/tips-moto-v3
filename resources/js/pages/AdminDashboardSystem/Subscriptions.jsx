...
const generatePackageTips = (packageType, packageName, tipCount) => {
    ...
    for (let i = 0; i < tipCount; i++) {
        ...
        const getOddsForRisk = (riskLevel) => {
            switch (riskLevel) {
                case 'low':
                    return (1.15 + Math.random() * 0.65).toFixed(2); // 1.15 - 1.80
                case 'mid':
                    return (1.75 + Math.random() * 1.25).toFixed(2); // 1.75 - 3.00
                case 'high':
                    return (2.8 + Math.random() * 4.2).toFixed(2); // 2.80 - 7.00
                default:
                    return '2.10';
            }
        };
...
const generateAnalysisReason = (tipType, match) => {
    const reasons = {
    ...
    const tipReasons = reasons[tipType] || ['Professional analysis suggests this outcome'];
    return tipReasons[Math.floor(Math.random() * tipReasons.length)];
};

// Helper function to get confidence level
const getConfidenceLevel = (riskLevel) => {
    switch (riskLevel) {
        case 'low':
...
const calculateExpectedReturn = (odds) => {
...
// Filter subscriptions based on search and filters
    const filteredSubscriptions = subscriptions.filter((subscription) => {
        ...
        const matchesStatus =
            statusFilter === "all" ||
            (statusFilter === "true" && subscription.is_active === true) ||
            (statusFilter === "false" && subscription.is_active === false);

        const matchesDescription =
            descriptionFilter === "all" ||
            subscription.features.category === descriptionFilter;
    ...
    const getStatusBadge = (is_active) => {
        return is_active ? <Badge className="bg-green-600 text-white">Active</Badge> : <Badge variant="secondary">Inactive</Badge>;
    };
    ...
    const handleViewPackage = (subscription) => {
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <Button onClick={handleShowAddPackage} className="flex items-center space-x-2 bg-blue-600 text-white hover:bg-blue-700">
                                ...
                                {currentSubscriptions.length === 0 ? (
                                    ...
                                    currentSubscriptions.map((subscription) => (
                                        <TableRow key={subscription.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                            ...
                                            <TableCell>{getStatusBadge(subscription.is_active)}</TableCell>
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-between border-t px-6 py-4">
                            <div className="text-sm text-gray-500 dark:text-gray-400">
                                Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredSubscriptions.length)} of{' '}
            ...
            {/* Add Package Dialog */}
            <AddPackageDialog open={showAddPackageDialog} onClose={handleCloseAddPackage} onAddPackage={handleAddPackage} />
        </div>
    );
}
